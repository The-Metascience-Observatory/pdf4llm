...
@contextmanager
def _suppress_c_stderr() -> Iterator[None]:
    """
    Temporarily redirect C-level stderr output to /dev/null.

    MuPDF emits some warnings directly from native code, bypassing Python logging.
    """
    try:
        stderr_fd = sys.stderr.fileno()
    except (AttributeError, OSError):
        # If stderr doesn't expose a file descriptor, continue without suppression.
        yield
        return
...
def extract_with_ocr(pdf_path: Path, config: Optional[Config] = None) -> DocumentModel:
    ...
    # Suppress MuPDF native warnings during document open.
...
def _extract_sections_ocr(text: str) -> List[Section]:
...
def _extract_references_ocr(text: str) -> list:
    ...
    for i, entry in enumerate(ref_entries):
        ...
        # Store full raw text for later GROBID re-parsing; use it as
        # a fallback title until structured parsing can be done.
        clean = re.sub(r'\s+', ' ', entry).strip()
        references.append(Reference(
            num=i + 1,
            raw_text=clean,
            title=clean[:200] if len(clean) > 200 else clean,
            doi=doi,
        ))
